...
#include "LAV_OutputPin.h"
#include "LAV_InputPin.h"
...
STDMETHODIMP CLAVSplitter::LoadDefaults()
{
	...
	m_settings.QueueMaxPackets = 0;
	m_settings.QueueMaxMemSize = 0;
...
STDMETHODIMP CLAVSplitter::ReadSettings(HKEY rootKey)
{
	...
	if (SUCCEEDED(hr)) {
		...
		if (SUCCEEDED(hr)) m_settings.TrayIcon = FALSE;//bFlag;
